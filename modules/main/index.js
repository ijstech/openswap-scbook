    ...
    let DocsSite = class DocsSite extends components_2.Module {
        constructor(parent, options) {
            ...
            if (window.location.pathname.startsWith('/ipfs/')) {
                this.rootUrl = window.location.pathname.substring(0, 65);
            }
            else {
                this.rootUrl = '';
            }
        ...
        addBtnEvent() {
            ...
            (_b = Array.from(editBtns)) === null || _b === void 0 ? void 0 : _b.map((btn) => btn.addEventListener('click', async (e) => {
                ...
                const request = new Request(`${this.entrypoint || this.rootUrl}/${filePath}`);
                const response = await fetch(request);
                const script = await response.text();
        ...
        async processGitbook(content, filePath) {
            ...
            if (fileRegex.test(content)) {
                const sclinkPath = `${this.entrypoint || this.rootUrl}/${filePath}`;
                ...
                if (result.ok) {
                    ...
                    content = content.replace(fileRegex, (_, src) => {
                        ...
                        const link = `${this.entrypoint || this.rootUrl}/${src.replace('../', '')}`;
        ...
        async init() {
            ...
            this.docsHeader.rootUrl = this.rootUrl;
            this.docsNavigator.rootUrl = this.rootUrl;
            this.docsPaging.rootUrl = this.rootUrl;
        ...
        async loadMenu() {
            return new Promise(async (resolve, reject) => {
                ...
                if (this.forceUpdate || !((_a = window.localStorage) === null || _a === void 0 ? void 0 : _a.getItem('$$scbook_menu'))) {
                    ...
                    const response = await this.retryLoadFile(`${this.entrypoint || this.rootUrl}/SUMMARY.md`);
        ...
        async loadContent(file, slug) {
            return new Promise(async (resolve, reject) => {
                ...
                if (this.forceUpdate || !((_a = window.localStorage) === null || _a === void 0 ? void 0 : _a.getItem(`$$scbook_${slug}`))) {
                    const response = await this.retryLoadFile(`${this.entrypoint || this.rootUrl}/${file}`);
                ...
                text = text.replace(regex, (string, filename) => {
                    if (string.indexOf('http://') >= 0 || string.indexOf('https://') >= 0)
                        return string;
                    const newFileName = `${this.entrypoint || this.rootUrl}/${encodeURIComponent(filename.replace('/..//g', '').replace('/\\/g', ''))}`;
